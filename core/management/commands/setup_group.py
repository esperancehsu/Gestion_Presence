# users/management/commands/auto_assign_groups.py
...
class Command(BaseCommand):
    ...
    def create_permissions(self, dry_run=False):
        ...
        for codename, name, content_type in permissions_to_create:
            if dry_run:
                exists = Permission.objects.filter(codename=codename, content_type=content_type).exists()
                status = "existe" if exists else "sera créée"
